...
| `[x]` | Agent loop (understand/plan/execute/observe) | 2026-03-21 — same as 2026-03-19 baseline, plus each step system prompt can append **Agent task status (runtime):** from `session.metadata["agent_task_state"]` while the task is active (`localagentcli/session/task_context.py`, `AgentLoop.run(..., session=...)`) |
