...
| `[x]` | `/set` target-selection command | 2026-03-25 — unified picker for local models and provider models, with provider model selection starting empty instead of prefilled and picker descriptions surfacing readiness tiers and discovery state; legacy-fallback provider selections add explicit post-selection guidance to refresh discovery and pick an API-discovered model; shared prompt helpers now run loop-safely inside the async shell runtime so `/set` no longer crashes on nested event-loop errors |
