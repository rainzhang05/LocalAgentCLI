...
> **Last updated**: 2026-04-02 — **Phase 16 slice 5 baseline (guardian reviewer, in progress):** safety config adds `safety.approvals_reviewer` (`user` / `guardian_subagent`), agent loop approval routing can run eligible actions (`shell_execute`, `file_write`, `patch_apply`, mutating `mcp__*`) through guardian risk review with fail-closed denial, guardian lifecycle events are rendered in shell activity output, runtime/controller task-state now persists reviewer + latest guardian decision metadata, and targeted regressions cover reviewer parsing, loop/controller integration, status output, runtime controller-cache invalidation, and headless JSON runtime behavior.

> **Last updated**: 2026-04-01 — **Phase 17 follow-on (mock-provider E2E, shipped):** `tests/e2e/test_phase17_session_lifecycle.py::test_e2e_exec_chat_mock_remote_provider_save_and_subprocess_reload` adds deterministic RemoteProvider-path coverage for headless `exec` with `--save-session`, validating saved-session persistence plus subprocess `/session load` round-trip.
>
> **Last updated (previous)**: 2026-04-01 — **Phase 17 slices 10–11 (shipped):** `test_headless_exec_json_mode_save_session_persists_history` asserts headless `exec --json` with `--save-session` writes a loadable session file whose `history` includes both `user` and `assistant` roles. `.github/workflows/perf.yml` is a manual **Perf** workflow (`workflow_dispatch`) that runs `RUN_PERF=1 python -m pytest tests/perf/ -v -m perf` on Ubuntu with Python 3.11 (aligned with `release_verify`); `docs/packaging-and-release.md` notes maintainers can trigger it for opt-in CI perf visibility.
>
...
| `[x]` | `/status` command | 2026-04-02 — `/status` and the prompt toolbar still share one snapshot contract; status output now includes reviewer mode and latest guardian decision/risk summary (when present) in addition to task timing (`active`, `started_at`, `updated_at`, computed elapsed) and route/phase/step/pending-tool/retry/error/undo-ready fields |
...
| `[x]` | Config defaults and validation | 2026-04-02 — safety config includes reviewer routing validation (`safety.approvals_reviewer = user|guardian_subagent`) in addition to OS-wrapper backend validation (`off`/`auto`/`macos-seatbelt`/`linux-bwrap`/`container-docker`), typed sandbox policy overrides (`safety.sandbox_network_access`, `safety.sandbox_writable_roots`), and optional container backend image/resource settings |
...
| `[x]` | Agent loop (understand/plan/execute/observe) | 2026-04-02 — includes guardian reviewer routing for eligible approval-required actions (`shell_execute`, `file_write`, `patch_apply`, mutating `mcp__*`) when `safety.approvals_reviewer = guardian_subagent`, with fail-closed denial and explicit guardian lifecycle events; existing step/runtime/truncation/unified-loop behavior remains intact |
...
| `[x]` | Agent events system | 2026-04-02 — includes guardian review lifecycle events (`guardian_review_started`, `guardian_review_completed`) rendered in shell activity lanes alongside existing route/phase/plan/step/tool/completion/timeout/failure events |
...
| `[x]` | Approval manager (balanced mode) | 2026-04-02 — central safety gate enforces prompts for standard side-effecting actions and read-only high-risk actions; reviewer routing can now delegate eligible approval-required actions to guardian review when configured |
...
| `[x]` | `docs/agent-system.md` | 2026-04-02 — documents guardian review lifecycle events, reviewer routing in `waiting_approval`, and expanded task-state fields (`approvals_reviewer` + latest guardian decision metadata) alongside class-aware retry/replan policy |
