...
| `[x]` | Model installer (HF download) | 2026-03-22 — Hugging Face Hub download with live per-file progress when dry-run planning is available, width-safe labels for narrow terminals, JSONL telemetry sidecar entries (`cache/downloads/install_telemetry.jsonl`), and post-install summary output (source/time/downloaded/cached/average speed/files) |
| `[x]` | Model installer (URL download) | 2026-03-22 — httpx streaming with resume support, width-safe progress labels, failure-and-success telemetry records in `install_telemetry.jsonl`, and the same post-install summary output contract as HF installs |
