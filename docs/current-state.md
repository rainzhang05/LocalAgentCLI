...
> **Last updated**: 2026-03-31 — **End-to-end validation (shipped):** `tests/e2e/test_phase17_session_lifecycle.py` exercises headless `exec` with deterministic local model stubs (`ModelBackend` implementations): agent mode performs a read-only `file_read` tool round then completes, `--save-session` persists JSON, and a second CLI subprocess loads the session and runs `/status`. Chat-mode `exec` is covered with a text-only stub. Tests use `await _run_exec_async(...)` with a fast-path patch for local `astream_generate` to keep CI latency low.
>
...
| `[x]` | `docs/current-state.md` | 2026-03-29 — updated with Phase 16 slice 6 retry/replan policy + failure-type task-state surfaces |
