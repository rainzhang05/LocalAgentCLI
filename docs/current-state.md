...
>
> **Phase 16 slice 4 baseline increment (in progress, 2026-03-29):** path-based multi-agent groundwork now includes a validated `AgentPath` primitive (`localagentcli/agents/agent_path.py`), shared target resolver (`resolve_agent_reference(...)`), and a feature-gated in-process multi-agent manager (`localagentcli/agents/multi_agent.py`) wired into runtime dynamic tools when `features.multi_agent_path_routing=true`. The baseline dynamic tool surface now includes `spawn_agent`, `send_input`, `wait_agent` (plus `wait` alias), `close_agent`, and `resume_agent`, with lightweight active-agent metadata snapshots persisted under `session.metadata["active_agents"]`. Sub-agent workers now execute model-backed turns when an active model is available, with deterministic fallback when no model is connected. Multi-agent lifecycle state now includes generation-guarded thread updates so stale worker threads cannot overwrite resumed-agent status. Coverage includes `tests/test_agent_path.py`, `tests/test_feature_registry.py`, `tests/test_multi_agent_manager.py`, and runtime integration assertions in `tests/test_runtime_core.py`.
>
